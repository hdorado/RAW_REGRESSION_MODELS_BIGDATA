...
varImportance <- function(model, pred.data=model$trainingData, ..., scale=T) 
{
  ...
  for(input in names(inputs)) {
    ...
    if (is.factor(xv) && !is.ordered(xv)) {
      ...
      mode.err <- as.numeric(postResample(predict(model, x.data), response)[1])
      for (i in seq(along = x.pt)) {
        x.data <- pred.data
        x.data[, xname] <- factor(rep(x.pt[i], n), levels = x.pt)
        input.cancellation <- (as.numeric(postResample(predict(model, x.data), response)[1]) - mode.err)^2
      ...
      mean.err <- as.numeric(postResample(predict(model, x.data), response)[1])
      for (i in seq(along = x.pt)) {
        x.data <- pred.data
        x.data[, xname] <- rep(x.pt[i], n)
        input.cancellation <- (as.numeric(postResample(predict(model, x.data), response)[1]) - mean.err)^2
        if(!is.na(input.cancellation)) err <- err + input.cancellation
      }
    }
    errors[input,] <- err
